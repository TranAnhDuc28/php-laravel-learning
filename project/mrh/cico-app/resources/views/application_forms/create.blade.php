            ...
            <div class="col-md-8">
                <div class="card" style="border-color: #0d6efd;">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h6 class="mb-0">Create Application Form</h6>
                    </div>
                    <div class="card-body">
                        <x-input-error :messages="$errors->all()" class="mt-2" />
                        <div class="mb-3">
                            <p>Avaiable paid leave to use:</p>
                            <p>- Remaining paid leave of previous year: {{ $leaveDayLog->days_off_to_june }}</p>
                            <p>- Paid Leave In Advance: {{ $leaveDayLog->days_off_in_advance }}</p>
                            <p>- Paid Leave: {{ $leaveDayLog->days_off + $leaveDayLog->award_days_off + $leaveDayLog->carried_days_off}}</p>
{{--                            @if($isNewYear)--}}
{{--                                <p>Avaiable to use:</p>--}}
{{--                                <span>Remaining paid leave of previous year: {{ $leaveDayLog->days_off_to_june }}</span>--}}
{{--                                <span>Paid Leave In Advance: {{ $leaveDayLog->days_off_in_advance }}</span>--}}
{{--                                <span>Paid Leave: {{ $leaveDayLog->days_off + $leaveDayLog->award_days_off + $leaveDayLog->carried_days_off}}</span>--}}
{{--                            @else--}}
{{--                                <p>Avaiable to use:</p>--}}
{{--                                <span>Remaining paid leave of previous year: {{ $leaveDayLog->days_off_to_june }}</span>--}}
{{--                                <span>Paid Leave In Advance: {{ $leaveDayLog->days_off_in_advance }}</span>--}}
...
{{--                            @endif--}}
                        ...
                        <form id="leaveForm" method="POST" action="{{ route('application-forms.store') }}">
                            @csrf
...
{{--                                        <option value="{{ $value }}">{{ $label }}</option>--}}
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" min="{{ $exceedDay }}"
                                       value="{{ old('start_date', isset($applicationForm) ? \Carbon\Carbon::parse($applicationForm->start_date)->format('Y-m-d') : $exceedDay) }}" class="form-control @error('start_date') is-invalid @enderror" id="start_date" name="start_date" required>
                            </div>

                            <div class="mb-3">
                                <label for="end_date" class="form-label">End Date</label>
                                <input type="date" min="{{ $exceedDay }}"
                                       value="{{ old('end_date', isset($applicationForm) ? \Carbon\Carbon::parse($applicationForm->end_date)->format('Y-m-d') : $exceedDay) }}" class="form-control @error('end_date') is-invalid @enderror" id="end_date" name="end_date" required>
                            </div>
...
{{--                                        @foreach(['00', '30'] as $minute)--}}
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('leave_reason') is-invalid @enderror" id="leave_reason" name="leave_reason" rows="3" required>{{ old('leave_reason', isset($applicationForm) ? $applicationForm->leave_reason : "") }}</textarea>
                            </div>
