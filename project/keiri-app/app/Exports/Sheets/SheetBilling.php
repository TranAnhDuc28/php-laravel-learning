...
class SheetBilling implements FromView, WithEvents, WithTitle, withStyles
{
    ...
    public function styles(Worksheet $sheet): void
    {
        ...
        //
        $sheet->getStyle('A7:J13')->getBorders()->getOutline()
            ->setBorderStyle(Border::BORDER_THIN);
        $sheet->getStyle('F7:F13')->getBorders()->getLeft()
            ->setBorderStyle(Border::BORDER_THIN);
        ...
        $sheet->getStyle('A8:J11')->getAlignment()
            ->setHorizontal(Alignment::HORIZONTAL_LEFT)
            ->setVertical(Alignment::VERTICAL_TOP)
            ->setWrapText(true);

        //
        $sheet->getStyle('A15:J21')->getBorders()->getOutline()
            ->setBorderStyle(Border::BORDER_THIN);
        $sheet->getStyle('F15:F21')->getBorders()->getLeft()
            ->setBorderStyle(Border::BORDER_THIN);
        ...
        $sheet->getStyle('A23:J29')->getBorders()->getAllBorders()
            ->setBorderStyle(Border::BORDER_THIN);
        $sheet->getStyle('A29:J23')->getFont()->setBold(true);
        $sheet->getStyle('A29:J23')->getAlignment()
            ->setHorizontal(Alignment::HORIZONTAL_CENTER)
            ->setVertical(Alignment::VERTICAL_CENTER)
            ->setWrapText(true);
        ...
        $sheet->getStyle('G34:J34')->getBorders()->getBottom()
            ->setBorderStyle(Border::BORDER_THIN);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                foreach ($this->titleReportDailySheets as $i => $iValue) {
                    $sheet->setCellValue('F' . ($rowAmountDataStart + $i), "=LOOKUP(9^9, +'$iValue'!H:H)");
                }
                $sheet->getStyle([6, $rowAmountDataStart, 10, $rowAmountDataTotal])->getNumberFormat()
                    ->setFormatCode('"¥"#,##0;"¥"-#,##0');
